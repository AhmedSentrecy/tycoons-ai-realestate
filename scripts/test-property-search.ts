...
import { parseSearchQuery, searchInventory } from "../src/lib/propertySearch.ts";
...
async function run() {
  ...
  const controlled = searchInventory(
    ...
    "شاليه في الساحل تحت 10 مليون",
  );
  assert.equal(controlled.totalExact, 1);
  assert.equal(controlled.exact[0]?.unit.starting_price, 9_000_000);
  assert.ok(controlled.alternatives.some((result) => result.differences.some((item) => item.includes("أعلى من الميزانية"))));
  ...
  const ivilla = searchInventory(units, "آي فيلا في التجمع 3 غرف");
  assert.ok(ivilla.exact.length > 0, "iVilla Arabic query should return exact matches");
  assert.ok(ivilla.exact.every((result) => /ivilla/i.test(result.unit.unit_type)));

  const budget = searchInventory(units, "شقة تحت 7 مليون");
  assert.ok(budget.exact.every((result) => result.unit.starting_price <= 7_000_000));
