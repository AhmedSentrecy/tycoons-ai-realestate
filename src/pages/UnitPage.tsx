...
export default function UnitPage() {
  ...
  useEffect(() => {
    ...
    const title = `${unit.unit_type} ${unit.area_sqm} م² في ${unit.project_name} | Tycoons`;
    const description = `${unit.unit_type} في ${unit.project_name} بسعر يبدأ من ${formatPrice(unit.starting_price)} جنيه. اعرف المساحة وخطة السداد والتشطيب.`;
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: unit.unit_type, item: pageUrl },
        ],
      },
    ],
  };
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#f7f2ea] text-[#1b2420]">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid items-end gap-8 lg:grid-cols-[1fr_auto]">
            <div>
              ...
              <h1 className="mt-5 text-4xl font-extrabold leading-tight sm:text-5xl">{unit.unit_type} {unit.area_sqm} م² في {unit.project_name}</h1>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><div className="grid gap-8 lg:grid-cols-[1fr_0.75fr]">
        <article><h2 className="text-3xl font-extrabold">تفاصيل الوحدة</h2><p className="mt-5 font-light leading-loose text-[#5c6a62]">{unit.description || `وحدة ${unit.unit_type} داخل ${unit.project_name} من ${unit.developer}.`}</p><div className="mt-8 grid gap-4 sm:grid-cols-2">{[
          [BedDouble, "الغرف", unit.bedrooms_text || "غير محدد"],
          [Maximize2, "المساحة", `${unit.area_sqm} م²`],
          [WalletCards, "خطة السداد", unit.installments_text || "حسب آخر تحديث"],
          [Building2, "التشطيب", unit.finishing || "حسب نوع الوحدة"],
