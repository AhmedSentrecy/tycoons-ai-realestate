...
import VoiceInquiryList from "./VoiceInquiryList";
...
export default function SalesWarRoomControlDashboard({scope}:{scope:ControlScope}){
  ...
  const tabs:Tab[]=manager?["overview","agents"]:["overview","analytics","pipeline","agents"];
  ...
  return <main className="min-h-screen bg-[#f3f5f7] text-slate-950" dir={lang==="ar"?"rtl":"ltr"}>
    {!manager && <details className="mx-auto max-w-6xl p-3"><summary className="cursor-pointer rounded-xl bg-emerald-900 p-3 font-bold text-white">عملاء المساعد الذكي — التسجيلات والملخصات</summary><VoiceInquiryList token={token} owner /></details>}
    <div className="mx-auto max-w-[1500px] p-3 pb-24 md:p-5">
      ...
      <nav className={`sticky top-2 z-30 mb-4 grid gap-2 rounded-2xl border bg-white/95 p-2 shadow-sm backdrop-blur ${manager?"grid-cols-2":"grid-cols-4"}`}>
      ...
      {!manager&&tab==="analytics"&&<SalesWarRoomMeetingAnalytics scope={scope} token={token} lang={lang}/>}
      ...
      {tab==="overview"&&<>
        ...
        <SalesWarRoomPeriodResults mode="control" scope={scope} token={token} lang={lang}/>
      ...
      {!manager&&tab==="pipeline"&&<section className="rounded-3xl border bg-white p-4 shadow-sm">
        ...
        {pipelineLoading&&!pipelineLoaded?<div className="p-10 text-center font-bold text-slate-400">{t("Loading pipeline…","جاري تحميل الـPipeline…")}</div>:<div className="mt-4 space-y-2">{filteredLeads.slice(0,visibleLimit).map((x:any)=>editingId===x.id&&leadDraft?<LeadEdit key={x.id} d={leadDraft} setD={setLeadDraft} lang={lang} t={t} saving={savingLead} onSave={()=>void saveLead()} onCancel={cancelEdit}/>:<div key={x.id} className="rounded-2xl border bg-slate-50 p-3"><div className="flex items-start justify-between gap-3"><div><div className="font-black">{x.client_name}</div><div className="mt-1 text-xs font-bold text-slate-500">{lang==="ar"?x.agent?.name_ar:x.agent?.name_en} · {lang==="ar"?stageAr[x.stage]:x.stage}</div></div><button onClick={()=>startEdit(x)} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">✏️ {t("Edit","تعديل")}</button></div><div className="mt-2 grid grid-cols-2 gap-2 text-xs"><div><b>{t("Expected","متوقع")}:</b> {money(Number(x.expected_value||0))}</div><div><b>{t("Follow-up","Follow-up")}:</b> {x.next_action_date?`${x.next_action_date}${x.next_action_time?` · ${String(x.next_action_time).slice(0,5)}`:""}`:"—"}</div></div><div className="mt-2 text-xs text-slate-600"><b>{t("Next","التالي")}:</b> {x.next_action||"—"}</div></div>)}{pipelineLoaded&&!filteredLeads.length&&<div className="p-10 text-center text-sm font-bold text-slate-400">{t("No matching leads.","مفيش Leads مطابقة.")}</div>}{filteredLeads.length>visibleLimit&&<button onClick={()=>setVisibleLimit(v=>v+50)} className="w-full rounded-xl border bg-white p-3 text-sm font-black">{t("Load 50 more","حمّل 50 كمان")} · {visibleLimit}/{filteredLeads.length}</button>}</div>}
      </section>}
