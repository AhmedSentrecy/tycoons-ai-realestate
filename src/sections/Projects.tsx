        ...
        <div className="mb-8 flex flex-wrap items-end justify-between gap-5">
          ...
          >
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-red-400/25 bg-red-400/10 px-3.5 py-1.5">
              ...
              <span className="text-xs font-semibold tracking-wide text-red-300">{tr("مباشر")}</span>
            </div>
            <h2 className="text-3xl font-extrabold text-white sm:text-4xl">{tr("بتتطرح دلوقتي")}</h2>
            <p className="mt-3 max-w-md text-sm font-light leading-relaxed text-white/60">
              {tr("أول فرصة لمشاريع لسه نازلة — من المطوّرين مباشرة، قبل السوق كله.")}</p>
          </motion.div>
        ...
        {projects.map((p, i) => (
          ...
          >
            ...
            <div className="relative aspect-[16/10] overflow-hidden">
              ...
              <span
                className={tr(`absolute right-4 top-4 rounded-full px-3.5 py-1.5 text-xs font-bold shadow-lg ${
                  p.badge === "جديد"
            ...
            <div className="p-4">
              <div className="text-xs font-medium text-[#d9b87c]">
                {tr(p.type)} · {p.developer}
              </div>
              <h3 className="mt-2 text-[17px] font-bold leading-snug text-white">{p.title}</h3>

              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-xs text-white/50">{tr("يبدأ من")}</span>
                <span className="text-lg font-extrabold text-[#ecd9ae]">{tr(p.price)}</span>
              </div>

              <div className="mt-3 grid grid-cols-3 gap-2 border-t border-white/10 pt-3 text-center">
                {[
                  { icon: BedDouble, v: p.beds, l: tr("غرف") },
                  { icon: Ruler, v: p.area, l: tr("المساحة") },
                  { icon: KeyRound, v: p.delivery, l: tr("الاستلام") },
                ].map(({ icon: Icon, v, l }) => (
                  <div key={l}>
                    <Icon className="mx-auto h-4 w-4 text-[#c49b5f]/80" />
                    <div className="mt-1.5 text-[13px] font-semibold text-white/90">{tr(v)}</div>
              ...
              <div className="mt-4 flex gap-2">
                <a
                  href={p.projectSlug ? `${isEnglish() ? '/en' : ''}/projects/${p.projectSlug}` : localeHref(arabicAreaPath(p.regionSlug))}
