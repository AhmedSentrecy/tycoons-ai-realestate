    ...
    <footer className="bg-[#08130e]">
      ...
      <div className="mx-auto max-w-6xl px-5 py-16 lg:px-8 lg:py-20">
        ...
        >
          ...
          <h2 className="relative mx-auto max-w-2xl text-balance text-2xl font-extrabold leading-snug text-white sm:text-4xl sm:leading-snug">
            {tr("عقارك الجاي يبدأ بجملة واحدة")}</h2>
          <p className="relative mx-auto mt-4 max-w-md text-sm font-light leading-relaxed text-white/65">
            {tr("ابحث بالذكاء الاصطناعي والصوت — من المطوّرين مباشرة، من غير وسطاء.")}</p>
          ...
          <div className="relative mt-6 flex items-center justify-center gap-2 text-xs text-white/45">
            ...
            {tr("يدعم البحث الصوتي بالعربي والإنجليزي")}</div>
        </motion.div>
      </div>
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-10 sm:grid-cols-2 lg:grid-cols-4 lg:px-8">
          <div>
            ...
            <p className="mt-5 max-w-xs text-sm font-light leading-relaxed text-white/50">
              {tr("ابحث عن عقارك بالذكاء الاصطناعي والصوت — من المطوّرين مباشرة.")}</p>
          </div>

          <div>
            <h4 className="mb-5 text-sm font-bold tracking-wide text-[#d9b87c]">{tr("المنصة")}</h4>
            <ul className="space-y-3.5">
              {[
                { label: tr("ابدأ البحث"), to: "/" },
                { label: tr("دليل المشاريع"), to: localeHref("/ar/"), external: true },
                { label: tr("الأسئلة الشائعة"), to: "/faq" },
                { label: tr("من نحن"), to: "/about" },
                { label: tr("الحاسبة"), to: "/" },
              ].map((it) => (
                <li key={it.label}>
                  {it.external ? (
          ...
          <div>
            <h4 className="mb-5 text-sm font-bold tracking-wide text-[#d9b87c]">{tr("المناطق")}</h4>
            <ul className="space-y-3.5">
              {regions.map((r) => (
                <li key={r.slug}>
                  <a
          ...
          <div>
            ...
            <ul className="space-y-3.5">
              {[
                { label: tr("من نحن"), to: "/about" },
                { label: tr("الأسئلة الشائعة"), to: "/faq" },
                { label: tr("منهجية البيانات"), to: "/methodology", external: true },
                { label: tr("سياسة التصحيح"), to: "/corrections", external: true },
                { label: tr("تواصل معنا"), to: "https://wa.me/201200704344", external: true },
              ].map((it) => (
                <li key={it.label}>
                  {it.external ? (
                    <a
            ...
            <details id="voice-privacy" className="mt-5 text-sm leading-7 text-white/70" dir={isEnglish() ? "ltr" : "rtl"}>
              <summary className="cursor-pointer font-bold text-[#d9b87c]">{tr("الخصوصية والمحادثات الصوتية")}</summary>
              <div className="mt-3 space-y-3">
                <p>{tr("المساعد الصوتي يعمل بالذكاء الاصطناعي. يُستخدم صوتك لمعالجة طلبك والرد عليك، وقد يخطئ في فهم الكلام؛ يُرجى تأكيد الأرقام والتفاصيل المهمة. عدم حفظ تسجيل لدينا لا يعني عدم معالجة الصوت لدى مزوّد الخدمة.")}</p>
                <p>{tr("أثناء المحادثة يُجهّز تسجيل صوتي ونص مؤقتان على جهازك. بعد إنهائها يمكنك مراجعة بيانات التواصل ونص الطلب ثم تأكيد إرسالهما للفريق، وإرفاق الصوت اختياريًا. إغلاق النموذج دون إرسال لا يرفع هذه البيانات إلى لوحة المبيعات. قد لا يكتمل التسجيل عند إغلاق الصفحة أو انقطاع الاتصال.")}</p>
                <p>{tr("الغرض من الحفظ هو متابعة طلبك ومراجعة جودة الخدمة، مع ربط التسجيل وملخص الاحتياجات بملف العميل. تُحفظ التسجيلات في مساحة خاصة على Supabase، ويقتصر الوصول على صاحب الحساب والموظف المسؤول بعد ربط الطلب بكارت العميل، عبر روابط مؤقتة. تتوقف إتاحة تشغيل الصوت بعد 30 يومًا؛ الحذف الدائم يتم بالمراجعة أو بطلب منك، وليس تلقائيًا حاليًا.")}</p>
                <p>{tr("مشاركة الاسم ورقم التواصل اختيارية، ولا تعني الموافقة على رسائل تسويقية. لا تشارك بيانات بطاقات الدفع أو كلمات المرور في المحادثة. يمكنك طلب توضيح بشأن بياناتك أو تصحيحها أو حذفها عبر")}<a className="underline" href="https://wa.me/201200704344">{tr("التواصل مع Tycoons")}</a>.</p>
              </div>
            </details>
          </div>
        </div>
        ...
        <div className="border-t border-white/10 py-6 text-center text-xs font-light text-white/40">
          {tr("© ٢٠٢٦ تايكونز للاستثمار — كل الحقوق محفوظة")}</div>
      </div>
    </footer>
  );
}
