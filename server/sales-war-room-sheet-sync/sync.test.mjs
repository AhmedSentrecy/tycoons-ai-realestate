...
test("a SODIC lead without feedback is never sent", () => {
  const headers = ["lead_id", "الاسم", "الموبايل", "وقت الليد", "راح لمين", "الحالة", "رد الـ CRM", "آخر إيجنت", "آخر فيدباك", "كل الفيدباك"];
  const map = warRoomHeaderMap_(headers);
  const bare = ["lead-1", "محمد", "01112345678", "2026-09-26 10:15", "CRM", "SENT", '{"data":{"id":23895}}', "Ahmed Yehya", "", ""];
  assert.equal(warRoomRow_(bare, map, { kind: "sodic" }), null);
  bare[8] = "تواصلنا وهنتابع بكرة";
  ...
  assert.equal(matched.lead_entered_at, "2026-09-26 10:15");
});
...
test("Hyde Park uses CRM agent and extracts only written feedback", async () => {
  const headers = ["crm_id", "الاسم", "الموبايل", "وقت الليد", "الإيجنت", "آخر كومنت", "آخر أكتيفيتي", "كل الكومنتات"];
  const map = warRoomHeaderMap_(headers);
  const row = ["23824", "محمد", "EG 01112345678", "2026-09-26 09:40", "Ahmed Yehya", "هنبعت التفاصيل", "2026-09-26 15:02", "2026-09-26 15:02 Ahmed Yehya: هنبعت التفاصيل"];
  ...
  assert.equal(mapped.lead_entered_at, "2026-09-26 09:40");
